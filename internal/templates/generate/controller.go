...
export class %sController {
  ...
  public async getAll(req: Request, res: Response): Promise<Response> {
    try {
      ...
      if (!%s) throw new Error("Résultat vide dans users");
...
`, cfg.Name, lowerName,
	...
	return fmt.Sprintf(`const %sService = require('../services/%s.service');

class %sController {
  constructor() {
    this.%sService = new %sService();
  }
...
`, cfg.Name, lowerName, cfg.Name, lowerName, cfg.Name, lowerName+"s", lowerName, lowerName+"s", lowerName)
}
